import { test, expect } from "@playwright/test";

test.describe("ブログ編集機能", () => {
  test("投稿済みの記事を編集し、変更が反映されること", async ({ page }) => {
    //編集対象の記事を作成する
    const uniqueId = crypto.randomUUID().slice(0, 8);
    const initialTitle = `編集対象-${uniqueId}`;
    await page.goto("/blog/add");
    await page.fill('input[name="title"]', initialTitle);
    await page.fill('textarea[name="description"]', "編集前の本文です。");

    await page.getByRole("button", { name: "投稿" }).click();

    await expect(page).toHaveURL("/");

    // 作成した記事の編集画面に遷移
    const blogItem = page.locator("li", { hasText: initialTitle });
    await blogItem.getByRole("link", { name: "編集" }).click();

    //既存データが表示されているか確認
    await expect(page.locator('input[name="title"]')).toHaveValue(initialTitle);

    //内容の書き換え
    const updatedTitle = `【更新済】${initialTitle}`;
    const updatedDescription = "本文を書き換えました。";

    await page.fill('input[name="title"]', updatedTitle);
    await page.fill('textarea[name="description"]', updatedDescription);

    //保存
    await page.getByRole("button", { name: "更新" }).click();

    // 古いタイトルが消え、新しいタイトルが表示されていることを確認
    await expect(page).toHaveURL("/");
    await expect(page.locator("body")).toContainText(updatedTitle);
    await expect(page.locator("body")).not.toHaveText(initialTitle);
  });
});
